...
- **Step 21-C (slim prompt) — BLOCKED.** Two gates, both must clear before starting:
  ...
  2. **Case-relevance pre-filter — PARTIALLY BUILT as of 2026-08-26** (WORKLOG.md, "gate 2: started the case-relevance pre-filter" entry). `caseStudyDomainMismatch` (`JobDetail.jsx`) is now fully derived from `CASE_LEDGER.service` instead of three hand-written name lists that had drifted badly out of sync with the ledger, and covers all three service axes (PPC/SEO/web-dev) instead of just PPC-vs-SEO. This is the ledger-matching mechanism §21-C's own plan calls for ("structured inputs — ledger case list for this job's domain"). **Still open:** the job-classification signal feeding it has a known hole — `WEBDEV_JOB_RE`'s bare platform-name alternatives (shopify/woocommerce/opencart/magento) fire on any mention with no build-intent check, confirmed as a false positive on a real job (13091, "Google Merchant Center Manager") that mentions "shopify" four times with zero build/dev asks. Needs its own dedicated fix (require a build-intent verb near those alternatives, same as the regex's other alternatives already do), verified against more than one real posting.
  Do not start 21-C until both gates fully clear — gate 1 is clear; gate 2's mechanism exists but its input signal still has a confirmed gap.
