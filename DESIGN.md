...
- **Step 21-B (grounding checker) — DONE in SHADOW (2026-07-27).** `frontend/src/lib/groundingCheck.js` — pure, deterministic, five §21.4 claim-classes: `metricNotInLedger`, `caseDuplicated`, `attachmentUnbacked`, `marketNotInPosting`, `seoAuditTurnaround`. Wired into both generate emit points via `_gcShadow()` behind a single `GC_ENFORCE` constant (**default `false` = shadow**: records violation codes to the `⚠ Top rule violations` telemetry, returns the letter UNCHANGED).
  - **Scoping (deliberate, false-positive discipline):** metric + attachment checks are scoped to **paragraphs that name a ledger case** — so pattern statements ("most accounts leak 30-40% of budget") are never touched. `metricNotInLedger` is **number-based** against the cited case's ledger figures. `marketNotInPosting` reads **only the posting body** (gazetteer-gated countries/demonyms/codes) — the client's account country is never a licence.
  - **Which defects the checker owns vs. the ledger owns:** the checker catches the **named-case** classes (job-8484 #4 metric, #5 dup-label, #6 turnaround; 2026-07-24 Israel/Danish market + duplicate block). The **anonymized/relabeled** forms (#1's unnamed story, #2 relabel, #3's anon half) are killed by the **ledger at 21-C** — once the prompt must emit `{{case:id}}`, an anonymized fabricated story or a relabeled vertical is unwriteable. "Catch all six" is a property of the **combined** system.
  - **Verified (bundled Node test, 12/12):** the job-8484 fixture fires `metricNotInLedger` + `caseDuplicated` + `attachmentUnbacked` + `seoAuditTurnaround`; the ledger line for skin-reboot has no Shopify/fabricated numbers and dedups to one block; the 2026-07-24 fixture fires `marketNotInPosting` (and does NOT when the posting authorises the market); a **CLEAN letter → ZERO flags** (incl. the "30-40%" pattern line and the PPC "1 working day" audit); shadow returns text unchanged; enforce strips the fabricated number / collapses the dup label.
  - **TO FLIP ENFORCE:** after the ~1-week shadow soak proves the panel counts match hand-review, set `GC_ENFORCE = true` (one line) — **owner check-in required first** (owner rule). Enforce actions are conservative: strip the untraceable metric number, `[market]` the unbacked geo, fix/dedup the attachment label; `caseDuplicated` + `seoAuditTurnaround` stay flag-only (dedup/strip handled structurally by the ledger and the existing `_stripSeoAuditTurnaround`).
  - Known limitation carried forward: dual-`service` cases (skin-reboot/derma are seo+ppc; ledger stores one primary) — a 21-C matching concern.
- **`GC_ENFORCE` flipped to `true` 2026-08-19** (commit `b8ecf2f`), after fixing 2 latent enforce-path bugs found while testing the flip (attachment-label rewrite firing unconditionally; metric/market enforcement leaving broken sentence fragments instead of removing the whole sentence). Same-day follow-up fixes: the chat-based proposal-rewrite path was bypassing `_gcShadow` entirely (fixed, commit `a507263`) and a new `tooManyCaseStudies` claim-class was added — so the clean-telemetry clock for 21-C's gate effectively restarts from 2026-08-19, not from the original 07-27 shadow ship.
- **Step 21-C (slim prompt) — BLOCKED, deliberately holding as of 2026-08-19 (owner decision: "let's soak for some time").** Two gates, both must clear before starting:
  1. **Enforce soak** (§21.6): a real stretch (~1 week, matching the original shadow-soak bar) of `GC_ENFORCE=true` telemetry with no new violation shapes suggesting the strip logic itself is garbling text, collected AFTER the 2026-08-19 fixes above (earlier data is contaminated by the bypass bug).
  2. **Case-relevance pre-filter built first** (not yet started, scoped 2026-08-19): §21-C's own plan calls for the slim prompt to include "the structured inputs (ledger case list for this job's domain)" as part of what replaces the 312 prohibitions — that's the case-relevance pre-filter using `CASE_LEDGER`'s existing `vertical`/`service` fields, discussed but deferred the same day. Cutting the prohibition text (including whatever currently nags the model about case relevance, however unreliably) before this structural replacement exists would remove a guardrail for the single biggest recurring manual-flag theme (wrong/irrelevant case choice — 6 of 17 owner flags since 08-04) with nothing standing in for it.
  Do not start 21-C until both gates clear — check `⚠ Top rule violations (30d)` telemetry and confirm the pre-filter exists before touching the prompt.
